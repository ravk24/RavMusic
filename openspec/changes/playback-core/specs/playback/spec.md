...
Tapping a song in a folder SHALL replace the queue with that folder's songs in their displayed order, starting at the tapped song, and start playing. Each queued item SHALL carry the song's title, artist and origin so that the current song and "Playing from: <folder name>" can be shown. Songs that follow SHALL play automatically in order; after the last song playback SHALL stop at the end of the queue (no repeat in this change).
...
The app SHALL expose the current song (title, artist, origin), whether playback is playing, the current position and duration, and whether a queue is loaded, updated as they change from any source (app, notification, headset). Position SHALL be refreshed at least twice a second while playing and the app is visible.
