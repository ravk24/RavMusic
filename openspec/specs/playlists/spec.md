...
The Playlists tab SHALL show every playlist as a card in a two-column grid, each with a deterministic gradient art tile, the name and "N songs · <total duration>" (total formatted as "2h 58m" or "51m"). The header SHALL show the title "Playlists" and the playlist count. A "+" action SHALL create a new playlist. When there are no playlists an empty state SHALL explain how to make one and offer the create action. Tapping a card SHALL open the playlist detail.
...
- **THEN** the detail shows "0 songs" and a hint to add songs from Folders; Play and Shuffle play do nothing
